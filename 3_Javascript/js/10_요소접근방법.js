...
function nameTest(){
    ...
    for(let i = 0; i<hobbyList.length; i++) { // 순차 접근
        ...
        if(hobbyList[i].checked){
            // hobbylist의 i번째 요소가 체크가 되어있으면
            str +=hobbyList[i].value +" ";
            // hobbylist의 i번쨰 요소의 value
            count++;
        }
...
function cssTest() {
    ...
    //querySelector -> 선택자가 선택한 요소 중 첫 번재 요소 반환하기 때문에
    //                  첫 번째 선택은 first-child 안 써줘도 됨

    first.innerText = "CSS 선택자로 선택해서 값 변경됨"
    ...
    // 두 번째 자식 div 접근해서
    const last = document.querySelector("[target-div='css-div'] > div:last-child");
    last.innerText = "첫 번째 요소가 아니면 querySelector() 특징 활용 못함";
...
// keydown : 키가 눌러졌을 때 이벤트 발생 (+ 꾹 누르고 있으면 계속 인식됨)
// keyup : 눌러지던 키가 떼어졌을 때 인식 (1회만 인식)

document.querySelector("#user-input")
.addEventListener("keyup",function(e){ 
    // e : (매개변수 e) : 이벤트 객체 (발생한 이벤트 정보를 담고 있는 객체)
    console.log(e);

    if(e.key == "Enter"){ // 엔터가 눌러졌을 때
        readValue(); //readValue호출
    }
})
