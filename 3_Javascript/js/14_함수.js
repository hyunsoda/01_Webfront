...
//클래스가 arrow인 요소를 모두 얻어옴 (배열 형태로 넘어옴)
...
arrowList[1].addEventListener("click",e => {
    ...
    print3(arr => {
        ...
        // 배열의 모든 요소들을 합쳐주기
        for(let i = 0; i<arr.length; i++){
            result += arr[i]; //arr의 i번째 인덱스 누적
        }

        return result; //result에 누적된 값을 return
        //안에 정의된 내용이 otherFn
    });
})
...
function twoNumberPlus(otherFn){
    ...
    alert( otherFn(input1, input2)); // alert도 하나의 함수. 기능임 (전달인자를 받을 소괄호가 있다)
}

arrowList[2]. addEventListener("click",() => { // 매개 변수 전달할 게 없는 경우도 생략 불가능

    twoNumberPlus((a,b) =>  a + b); // {return a+b}
    // return 한 줄만 있는 경우 
    // {}, return 생략 가능
});
...
function printObject(otherFn) {
    const obj = otherFn("홍길동", 20 );  //{"name":"홍길동", "age":"20"}으로 대입
...
arrowList[3].addEventListener("click", () => {
    ...
    printObject((name,age) => {
        ...
        return {"name":name, "age" : age}; //key value형태로 return
        // JS객체 {K:V, K:V}
        // return 제거 불가
...
(()=>{
    ...
    //버튼을 누르거나 호출하지 않았는데도 바로 호출됨 , 뒤에 붙은 소괄호가 호출부라 생각하면 됨
